...
**Рабочая директория:** все команды ниже выполняются из корня репозитория `mle-project-sprint-4-v001`. Если нужно перейти в подкаталог, используйте `cd` с соответствующим путём (в этом проекте отдельной папки `service` нет — код лежит в корне репозитория).
